(function() {
    ...
    trainService.$inject = ['$http', 'ENDPOINTS_CONSTANTS', 'distanceService', '$q'];

    function trainService($http, ENDPOINTS_CONSTANTS, distanceService, $q) {
        ...
        function getAllStations() {
            let request = {
                url: `irishrail/${ENDPOINTS_CONSTANTS.GET_ALL_STATIONS}`
            };
            return $http(request);
        }
        ...
        function getCurrentTrains() {
            let request = {
                url: `irishrail/${ENDPOINTS_CONSTANTS.GET_CURRENT_TRAINS}`
            };
            return $http(request);
        }
        ...
        async function orderStationsByDistance(stations, currentPosition) {
            ...
            // to prevent google to block us for flood 
            const size = 100;
            const TIMEOUT_GOOGLE_APIS_REQUESTS = 6000;
